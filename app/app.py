...
from scoring_engine import (
    ...
    get_actionable_solutions,
    get_rating_split,
)
from excel_exporter import export_from_db
...
def load_google_data():
    ...
    if not rest_url or not rev_url:
        logger.error("RESTAURANTS_SHEET_URL or REVIEWS_SHEET_URL not set in .env")
        return None, None, None
    try:
        return load_from_google_sheets(rest_url, rev_url)
    except Exception as e:
        logger.error("Error loading Google Sheets: %s", e)
        return None, None, None
...
# Persist score snapshot — throttled to once per restaurant per day.
# Without the throttle this writes a new row on every Streamlit rerender.
_score_date_key = f"_last_score_date_{restaurant_id}"
_today_str = datetime.now().strftime("%Y-%m-%d")
if st.session_state.get(_score_date_key) != _today_str:
    try:
        ...
        st.session_state[_score_date_key] = _today_str
    except Exception as _sh_err:
        logger.debug("save_score_history non-fatal: %s", _sh_err)

# compute_all_ranks is O(n²) — cache results in session state for this restaurant.
# Recompute only when the restaurant or city filter changes.
_ranks_cache_key = f"_ranks_{restaurant_id}_{_city}"
if _ranks_cache_key not in st.session_state:
    st.session_state[_ranks_cache_key] = compute_all_ranks(df_rest, df_rev)
df_ranks_all = st.session_state[_ranks_cache_key]

cur_rank = int(df_ranks_all[df_ranks_all["name"] == selected]["rank"].values[0])
total    = len(df_ranks_all)

# Filtered leaderboard — reuse all-ranks data if filter is "All Cities" with no extra filters.
_no_extra_filters = (
    st.session_state.min_rating_filter == 0.0
    and st.session_state.min_reviews_filter == 0
    and st.session_state.min_response_filter == 0
    and (_city == "All Cities" or not _city)
)
if _no_extra_filters:
    df_ranks = df_ranks_all
else:
    try:
        ...
        logger.warning("compute_all_ranks (filtered) failed: %s", _e)
...
# ============================================================
    ...
    if user_input or st.session_state.get("pending_question"):
        ...
        try:
            ...
            logger.warning("save_chat_session failed: %s", _e)
        st.rerun()
    ...
    if st.session_state.chat_messages:
        ...
        if last_ai:
            ...
            if len(st.session_state.chat_messages) >= 2:
                ...
                if last_user and last_ai:
                    # Cache by message count so we don't re-call the API on every rerender.
                    _sim_key = f"_similar_q_{len(st.session_state.chat_messages)}"
                    if _sim_key not in st.session_state:
                        with st.spinner("Generating related questions..."):
                            st.session_state[_sim_key] = get_similar_questions(
                                last_user["content"], last_ai["content"][:800],
                                selected, gaps, language=st.session_state.language,
                            )
                    similar = st.session_state[_sim_key]
                    if similar:
        ...
        if st.button("🗑️ Clear Chat", use_container_width=True):
            ...
            try:
                ...
                logger.warning("clear_chat_session failed: %s", _e)
            st.rerun()
    else:
...
# ============================================================
    ...
    with col_exp1:
        if st.button("📊 Export Master Excel", use_container_width=True, key="export_btn"):
            try:
                all_calls = get_all_call_notes_for_export()
                excel_bytes = export_from_db(all_calls)
                st.download_button(
                    "⬇️ Download Excel",
                    data=excel_bytes,
    ...
    if submitted:
        ...
        call_record = {
            ...
            "budget_range":       budget,
            "confidence":         confidence,          # DB column name
            "next_steps":         next_steps,
            "follow_up_date":     str(follow_up_date),
            "timeline":           decision_timeline,   # DB column name
            "competitor_tools":   competitor_tools,
            "products_discussed": products,
            "visit_outcome":      outcome,             # DB column name
            "notes":              notes,
            "image_data":         json.dumps(images_b64, ensure_ascii=False),  # DB stores as JSON string
        }
    ...
    if existing_calls:
        ...
        for i, call in enumerate(reversed(existing_calls), 1):
            ...
            outcome_val  = call.get("visit_outcome", "Pending")
            # image_data is stored as a JSON string in the DB
            raw_img = call.get("image_data", "") or ""
            try:
                call_images = json.loads(raw_img) if raw_img.strip().startswith("[") else []
            except Exception:
                call_images = []
            ...
            with st.container():
                ...
                m4.metric("Confidence", f"{call.get('confidence','—')}%")
                ...
                with col_d2:
                    st.markdown(f'<div style="font-size:11px;color:{NAVY};padding:6px 0"><b>Next Steps:</b> {call.get("next_steps","—") or "—"}<br><b>Follow-up:</b> {call.get("follow_up_date","—") or "—"}<br><b>Timeline:</b> {call.get("timeline","—") or "—"}</div>', unsafe_allow_html=True)
